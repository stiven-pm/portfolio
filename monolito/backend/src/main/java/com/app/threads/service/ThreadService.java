...
import com.app.threads.dto.*;
import com.app.threads.repository.ThreadMessageRepository;
import com.app.threads.repository.ThreadRepository;
...
public class ThreadService {
    ...
    public List<ChatThread> findByProject(UUID projectId) {
    ...
    public ThreadsPage findByProjectPaginated(UUID projectId, Integer limit, Integer offset) {
        if (limit == null || limit <= 0) {
            List<ChatThread> all = findByProject(projectId);
    ...
    public ThreadMessagesPage getMessagesPaginated(UUID threadId, Integer limit, Integer offset) {
        if (limit == null || limit <= 0) {
            ...
            List<ThreadMessageResponse> items = msgs.stream()
                    .map(m -> ThreadMessageResponse.from(m, threadId)).toList();
    ...
    @Transactional
    public ChatThread open(UUID projectId, UUID variantId, String type, UUID openedBy) {
    ...
    @Transactional
    public ChatThread close(UUID threadId, UUID closedBy) {
