...
function assertUploadSize(file, type) {
  const raw = file instanceof File ? file : file?.file;
  if (!raw?.size) return;
  const max = MAX_BYTES[type] ?? MAX_BYTES.default;
  if (raw.size > max) {
    const err = new Error(`Archivo demasiado grande (máx. ${Math.round(max / (1024 * 1024))} MB)`);
    err.code = 'FILE_TOO_LARGE';
    throw err;
  }
}

const CATALOG_IMAGE_MIMES = new Set(['image/jpeg', 'image/png', 'image/gif']);

function assertUploadMime(file, type) {
  const raw = file instanceof File ? file : file?.file;
  if (!raw || typeof raw.type !== 'string' || !raw.type) return;
  const mime = raw.type.toLowerCase();
  if (type === 'image' && !CATALOG_IMAGE_MIMES.has(mime)) {
    const err = new Error('Solo se admiten imágenes JPG, PNG o GIF');
    err.code = 'INVALID_MIME';
    throw err;
  }
  if ((type === 'plan' || type === 'pdf') && mime && mime !== 'application/pdf') {
    const err = new Error('Solo se admite PDF');
    err.code = 'INVALID_MIME';
    throw err;
  }
  if (type === 'model' && mime) {
    const ok =
      ...
      mime.includes('step') ||
      mime.includes('dwg');
    if (!ok && !mime.startsWith('image/')) {
      const err = new Error('Tipo de modelo no reconocido; use GLB, GLTF u otros admitidos');
      err.code = 'INVALID_MIME';
      throw err;
    }
  }
...
export async function uploadFile(file, type) {
  assertUploadSize(file, type);
  assertUploadMime(file, type);
  const formData = new FormData();
  formData.append('file', file instanceof File ? file : file.file || file);

  const res = await fetch(`${ENDPOINTS.document}/mediaFile/upload?type=${encodeURIComponent(type)}`, {
    method: 'POST',
    headers: {
...
function buildPdfHtml(data) {
  ...
  const productRows = (productos || []).map((p) => {
    ...
    const imgHtml = imagenUrl
      ? `<img src="${escapeImgSrcAttr(imagenUrl)}" class="thumb" alt=""/>`
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .muted{color:#6b7280;}
    .header-card{width:100%;border:1px solid #ede9fe;border-left:4px solid #7c3aed;background:#faf5ff;border-radius:6px;padding:12px 14px;margin-bottom:12px;}
    ...
    .t-final{background:#f3e8ff;color:#4c1d95;border-top:1px solid #7c3aed;font-weight:800;font-size:12px;}
